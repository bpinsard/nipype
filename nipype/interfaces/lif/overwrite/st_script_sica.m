function [sica,list_res] = st_script_sica(list_files,opt)
...
for numSess=1:nb_sess
    ...
    %%% Reading all runs
    [tmp,hdr] = st_read_vol(list_files_tmp{1}(1,:),[],0);
    hdr=hdr(1);
    clear tmp
    ...
    if isfield(opt,'mask') && ~isempty(opt.mask)
        ...
        for num_r = 1:length(list_files_tmp)
            ...
            mask{num_r} = st_segment_brain(data{num_r}(:,:,:,2:11))>0;
            [nx,ny,nz,nt]=size(data{num_r});
            data{num_r} = reshape(data{num_r},nx*ny*nz,nt);
            fprintf('Suppressing the first %i volumes !\n',suppress_vol);
            data{num_r}=data{num_r}(mask{num_r}(:)>0,suppress_vol+1:end);
        end

        % Computation of a mask of interest common to all runs
        mask_all = ones(size(mask{1}));
        for num_r = 1:length(list_files_tmp)
            mask_all = mask_all & mask{num_r};
        end
    end
    ...
    for num_r = 1:length(list_files_tmp)
        if exist('mask')
        	to_keep = mask_all(mask{num_r});
            data{num_r} = data{num_r}(to_keep,:)';
        else
        ...
        if slice_c == 1
            ...
            data{num_r} = st_correct_slice_intensity(data{num_r},mask_all);
        end
        if cut_hpfilter > 0
            fprintf('Temporal high-pass filtering of data (TR = %1.2f s, cut-off freq = %1.2fHz) \n',TR,cut_hpfilter)
            data{num_r} = st_filter_data(data{num_r},mask_all,TR,cut_hpfilter,'hp');
        end
        if cut_lpfilter > 0
            fprintf('Temporal low-pass filtering of data (TR = %1.2f s, cut-off freq = %1.2fHz) \n',TR,cut_lpfilter)
            data{num_r} = st_filter_data(data{num_r},mask_all,TR,cut_lpfilter,'lp');
        end
        if type_norm == 2
